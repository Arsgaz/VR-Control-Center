...
const journalTitle = computed(() => {
  return isCollapsed.value ? `Journal (${entries.value.length})` : 'User event journal'
})
</script>
...
<template>
  <footer class="event-log-panel" :class="{ collapsed: isCollapsed }">
    <div class="event-log-header">
      <div>
        <p class="section-label">Operator log</p>
        <h2>{{ journalTitle }}</h2>
      </div>
    ...
    <ol v-if="!isCollapsed && visibleEntries.length > 0" class="log-list">
      <li v-for="entry in visibleEntries" :key="entry.id">
        ...
        <div class="log-message">
          ...
          <small v-if="entry.deviceId">Device: {{ entry.deviceId }}</small>
        </div>
      </li>
    </ol>
  </footer>
